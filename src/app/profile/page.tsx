...
interface Photo {
  ...
  url: string;
  isPrimary: boolean;
  enhancedUrl?: string; // For enhanced version
...
export default function ProfilePage() {
  ...
  const [sliderPositions, setSliderPositions] = useState<Map<string, number>>(new Map());
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Add photos to profile (this triggers enhancement)
      setUploadStatus('Enhancing photos... (Analyzing image quality)');
      ...
      setTimeout(() => setUploadStatus('Enhancing photos... (Extending if cropped)'), 2000);
      setTimeout(() => setUploadStatus('Enhancing photos... (Correcting lighting & colors)'), 4000);
  ...
  const handleEnhancePreview = async (photoId: string, photoUrl: string) => {
    console.log(`✨ [PROFILE] Enhancing preview for photo: ${photoId}`);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.enhancedImageUrl) {
          ...
          setEnhancedImages(prev => new Map(prev).set(photoId, data.enhancedImageUrl));
          setSliderPositions(prev => new Map(prev).set(photoId, 50)); // Start at middle
        } else {
      ...
      console.error('❌ [PROFILE] Error enhancing image:', error);
      alert('Failed to enhance image. Please try again.');
    } finally {
          ...
          <div className="lg:px-6 lg:py-6 p-4 space-y-4">
            ...
            <div className="bg-white rounded-xl shadow-md border border-[#E8E8E6] p-4">
              ...
              {uploading && uploadStatus && (
                <div className="mb-4 bg-gradient-to-r from-[#8B5CF6]/10 to-[#7C3AED]/10 border border-[#8B5CF6]/30 rounded-lg p-3 flex items-center gap-3">
                  <Loader2 className="h-5 w-5 text-[#8B5CF6] animate-spin flex-shrink-0" />
                  <div className="flex-1">
                    <p className="text-sm font-medium text-[#1A1A1A]">{uploadStatus}</p>
                    <p className="text-xs text-[#5A5A5A] mt-0.5">
                      {uploadStatus.includes('Uploading') && 'Preparing your photo...'}
                      {uploadStatus.includes('Analyzing') && 'Checking image quality and dimensions...'}
                      {uploadStatus.includes('Extending') && 'Using AI to complete cropped images...'}
                      {uploadStatus.includes('Correcting') && 'Optimizing lighting and colors...'}
                    </p>
                  </div>
                </div>
              )}
            ...
            {/* Photo Enhancements - Automatic Display */}
            {profile && profile.photos.length > 0 && (
              <div className="bg-white rounded-xl shadow-md border border-[#E8E8E6] p-4">
                <h2 className="text-lg font-serif font-bold text-[#1A1A1A] mb-1 tracking-[-0.02em]">Photo Enhancements</h2>
                <p className="text-xs text-[#5A5A5A] mb-4 font-light">All uploaded photos are automatically enhanced for virtual try-on</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {profile.photos.map((photo, index) => {
                    const hasEnhancement = !!photo.enhancedUrl;

                    return (
                      <div key={photo.id} className="border border-[#E8E8E6] rounded-lg p-3 bg-[#FAFAF8]">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-medium text-[#1A1A1A]">Photo {index + 1}</span>
                            {photo.isPrimary && (
                              <span className="text-xs bg-[#8B5CF6] text-white px-2 py-0.5 rounded-full">Primary</span>
                            )}
                          </div>
                          {hasEnhancement && (
                        ...
                        {hasEnhancement ? (
                          <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <div>
                                <p className="text-xs text-[#5A5A5A] mb-1 font-medium">Original</p>
                                <div className="aspect-square rounded-lg overflow-hidden border border-[#E8E8E6]">
                                  <img src={photo.url} alt="Original" className="w-full h-full object-cover" />
                                </div>
                              </div>
                              <div>
                                <p className="text-xs text-[#5A5A5A] mb-1 font-medium">Enhanced</p>
                                <div className="aspect-square rounded-lg overflow-hidden border-2 border-[#8B5CF6] bg-gray-100">
                                  <img src={photo.enhancedUrl} alt="Enhanced" className="w-full h-full object-contain" />
                                </div>
                              </div>
                            </div>

                            {/* Enhancement details */}
                            <div className="bg-white rounded-lg p-2 border border-[#E8E8E6]">
                              <p className="text-xs font-medium text-[#1A1A1A] mb-1">Applied Enhancements:</p>
                              <div className="flex flex-wrap gap-1">
                                {photo.metadata?.enhancements?.lightingCorrected && (
                                  <span className="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full">Lighting</span>
                                )}
                                {photo.metadata?.enhancements?.qualityUpscaled && (
                                  <span className="text-xs bg-purple-50 text-purple-700 px-2 py-0.5 rounded-full">Upscaled</span>
                                )}
                                {photo.metadata?.enhancements?.imageExtended && (
                                  <span className="text-xs bg-green-50 text-green-700 px-2 py-0.5 rounded-full">Extended</span>
                                )}
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <p className="text-xs text-[#8A8A8A] text-center">No enhancement data available</p>
                          </div>
                        )}
                      </div>
                    );
