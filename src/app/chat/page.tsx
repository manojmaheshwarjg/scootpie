...
import { Send, Bot, User, RotateCcw, ExternalLink, Trash2, ChevronDown, MessageSquare, Clock, ImageOff } from 'lucide-react';
                        ...
                        <div className="max-h-[320px] overflow-y-auto space-y-1">
                          {conversations.map((c) => (
                            <button
                              ...
                              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-left group ${conversationId === c.id
                            ...
                            >
                              <div className={`flex h-10 w-10 items-center justify-center rounded-lg transition-all ${conversationId === c.id
                                  ? 'bg-gradient-to-br from-[#8B5CF6] to-[#7C3AED] shadow-sm'
                                  : 'bg-gray-100 group-hover:bg-[#8B5CF6]/10'
                                }`}>
                                <MessageSquare className={`h-[18px] w-[18px] ${conversationId === c.id ? 'text-white' : 'text-gray-500'
                                  }`} />
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className={`font-semibold text-sm truncate ${conversationId === c.id ? 'text-[#8B5CF6]' : 'text-[#1A1A1A]'
                                  }`}>
                                  Conversation
                                </div>
                          ...
                          {conversations.map((c) => (
                            ...
                            >
                              <div className={`flex h-10 w-10 items-center justify-center rounded-lg transition-all ${conversationId === c.id
                                ...
                                <MessageSquare className={`h-4 w-4 ${conversationId === c.id ? 'text-white' : 'text-gray-500'
                                  }`} />
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className={`font-semibold text-sm truncate ${conversationId === c.id ? 'text-[#8B5CF6]' : 'text-[#1A1A1A]'
