...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check for Gemini API key before attempting try-on
    // Check for Gemini API key before attempting try-on
    if (!process.env.GEMINI_API_KEY) {
      console.warn('[CHAT] GEMINI_API_KEY missing, skipping try-on generation');
      // Do not set responseText here, let standard response generation handle it
    } else if (itemsForTryOn.length > 0) {
      ...
      // Use pre-enhanced photo from database (only if using original photo, not prior outfit)
      let photoForTryOn = baseImage;
      if (baseImage === primaryPhoto.url) {
        // Check if we have a pre-enhanced version stored in database
        if (primaryPhoto.enhancedUrl) {
          console.log('[CHAT] Using pre-enhanced photo from database');
          photoForTryOn = primaryPhoto.enhancedUrl;
        } else {
          console.log('[CHAT] No pre-enhanced version, using original');
          // Fallback: Photo was uploaded before enhancement feature, use original
          photoForTryOn = primaryPhoto.url;
        }
      } else {
        console.log('[CHAT] Using prior outfit image, skipping enhancement');
      }

      try {
        const outfitResult = await generateOutfitTryOn(photoForTryOn, itemsToApply);
        ...
        if (outfitResult.success && outfitResult.imageUrl) {
          ...
          console.warn('[CHAT] Try-on returned success=false or no image');
          // Append fallback note to responseText later if needed
        }
      } catch (tryOnError) {
        console.error('[CHAT] Critical error during try-on generation:', tryOnError);
        // Do not fail the request, just log and continue without image
      }
    } else {
    ...
    if (mergedItems.length > 0) {
      ...
      // Append fallback note if try-on was attempted but failed (no image url generated)
